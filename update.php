...
// Query untuk mendapatkan data yang akan diperbarui
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  id_ruangan = '{$newData['Ruangan']}',
  id_dosen = (SELECT id FROM dosen WHERE nama = '{$newData['NamaDosen']}'),
  id_matkul = (SELECT id FROM mata_kuliah WHERE nama = '{$newData['Matkul']}'),
  id_kelas = '{$newData['Kelas']}',
  jam_mulai = CONCAT('{$newData['JamMulai1']}', ' - ', '{$newData['JamMulai2']}'),
  jam_akhir = CONCAT('{$newData['JamAkhir1']}', ' - ', '{$newData['JamAkhir2']}'),
  hari = '{$newData['Hari']}'
WHERE id = $id";
...
<body>
  ...
  <form method="post">
    ...
    <select name="nama_matkul" required>
      <?php
      // Fetch options for matkul from the database
    ...
    <select name="nama_kelas" required>
      ...
      $kelasQuery = "SELECT DISTINCT id AS 'ID KELAS', CONCAT(nama, ' ', semester) AS 'NAMA KELAS' FROM kelas k ORDER BY id";
